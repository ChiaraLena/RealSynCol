...
class Trainer:
    def __init__(self, options):
        ...
        print(self.opt.drop_path)
        print(self.opt.width, self.opt.height)
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, self.opt.name_custom_dataset, 4, is_train=True, img_ext=img_ext)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, self.opt.name_custom_dataset, 4, is_train=False, img_ext=img_ext)
    ...
    def process_batch(self, inputs):
        ...
        # keys_to_print = [('color', -1, 0), ('color', 0, 0), ('color', 1, 0)]
        # titles = ["color -1 0", "color 0 0", "color 1 0"]
        #
        # for ax, key, title in zip(axs, keys_to_print, titles):
        #     tensor = inputs[key]
        #     # Converti in numpy se è un tensore torch
        #     if hasattr(tensor, "detach"):
        #         img = tensor.detach().cpu().numpy()
        #     else:
        #         img = tensor
        #
        #     # Rimuovi dimensione batch se presente
    ...
    def compute_depth_losses(self, inputs, outputs, losses):
        ...
        depth_pred = torch.clamp(depth_pred, self.opt.min_depth, self.opt.max_dept)
